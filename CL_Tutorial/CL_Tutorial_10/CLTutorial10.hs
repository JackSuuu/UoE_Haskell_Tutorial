...
ddelta (FSM qs as ts ss fs) source sym = targetStates
  where
    transition = \s -> [t | (s', a, t) <- toList ts, s' == s, a == sym]
    targetStates = fromList [t | s <- toList source, t <- transition s]
...
dfinal fsm@(FSM qs as ts ss fs) = filterS (containsFinalState fsm)
                                  where
                                    containsFinalState :: (Ord q) => FSM q -> Set q -> Bool
                                    containsFinalState (FSM qs as ts ss fs) super =
                                      any (`member` fs) (toList super)
