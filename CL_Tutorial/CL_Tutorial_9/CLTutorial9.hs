...
--                states    symbols      transitions start   final
--                   Q      Sigma        delta       S        F
...
trace (FSM qs as ts ss fs) word = tr ss word  where
  tr ss' [] = [ss']
  tr ss' (w : ws) = ss' : tr (nextStates ss' w) ws
    where
      nextStates ss sym = foldr (\(q1, s, q2) acc -> if s == sym && q1 `member` ss then acc \/ singleton q2 else acc) empty ts
...
# 2
complement operation ¯ in DFAs does not always result in another DFA. 
Complementing a DFA may lead to a non-deterministic finite automaton (NFA) or even a non-regular language. 
Therefore, the closure property required for a boolean algebra is violated.
